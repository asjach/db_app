...
class PageBukuInduk(QWidget, Ui_Form):
    ...
    def fill_tbl_daftar_siswa(self):
        generate_table(
            data=self.data_buku_induk(),
            table=self.tbl_daftar_siswa
        )
    ...
    def data_buku_induk(self):
        ...
        if opsi_data == 'seluruh siswa':
            data = self.MODEL.get_all_siswa(search_by, search_text, order_by, self.opsi_kolom)
        else:
            data = self.MODEL.get_all_siswa_aktif(search_by, search_text, order_by, self.opsi_kolom)
        return data
    
    @property
    def opsi_kolom(self):
        kolom = BUKU_INDUK.get(self.parent.cbo_kolom.currentText().lower(), BUKU_INDUK['default'])
        return kolom
