...
class ConnectDB:
    ...
    def get_data(self, sql, params=None, return_fields=False):
        ...
        try:
            ...
            logger.exception("ERROR get_data")
    ...
    def get_list_data(self, sql, params=None):
        ...
        try:
            ...
            logger.exception("ERROR get_list_data")
            return []
        finally:
            if not self.in_transaction and self.my_connector:
                self.my_connector.close()
    ...
    def get_one_data(self, sql, params=None):
        ...
        try:
            ...
            logger.exception("ERROR get_one_data")
            return None
        finally:
            if not self.in_transaction and self.my_connector:
                self.my_connector.close()

    def get_table_columns(self, nama_tabel):
    ...
    def update_data(self, sql, params=None):
        try:
            ...
            logger.exception("ERROR update_data")
            return False
        finally:
            if not self.in_transaction and self.my_connector:
                self.my_connector.close()
